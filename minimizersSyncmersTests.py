...
def ratio_preserved_mers_per_error_rate(kmers, sim_read_kmers):
    ...
    for i in range(len(nr_shared)):
        ratios.append(2*nr_shared[i]/(len(kmers)+len(sim_read_kmers[i])))
...
def main():
    ...
    n = 5
    """for i in range(len(sim_reads_minis)):
        print(calculate_coverage(sim_reads_minis[i], 15, len(sim_reads[i])))
        print(calculate_coverage(sim_reads_syncs[i], 15, len(sim_reads[i])))"""
    ratios_mini = []
    ratios_sync = []
    avg_cover_mini = []
    avg_cover_sync = []
    for i in range(n):
        ...
        for j in range(len(sim_reads_minis)):
            cover_mini.append(calculate_coverage(sim_reads_minis[j], k, len(sim_reads[j])))
            cover_sync.append(calculate_coverage(sim_reads_syncs[j], k, len(sim_reads[j])))
